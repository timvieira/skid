...
- deleting document: is clunky, currently don't be removing directory (simple
  command-line interface can fix this) then droping the index and rebuilding it
...
- recommendation:
   ...
   - retrieve notes.
   - Probably want to modify ranking function

- better ranking function: preference for title, author, and notes
